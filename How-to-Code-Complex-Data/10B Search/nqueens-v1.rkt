...
(define (nqueens N)
  (local [
          (define (nqueens-bd bd)                  ;; Board -> Board or false
            (if (solved? bd)
                bd
                (nqueens-lobd (next-boards bd))))

          (define (nqueens-lobd lobd)              ;; (listof Board) -> Board
            (cond [(empty? lobd) false]
                  [else
                   (local [(define try (nqueens-bd (first lobd)))]
                     (if (not (false? try))
                         try
                         (nqueens-lobd (rest lobd))))]))

          (define (solved? bd) (= (length bd) N))

          ;; Board -> (listof Board)
          ;; produce boards with next generated queens
